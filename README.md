...
**Основные параметры:**
   ...
   1. Путь к серверу — папка, где находится ваш server.jar.

   2. Команда Java — обычно java или путь к конкретной версии (например, java17).

   3. Аргументы Java — флаги запуска, например: -Xmx2048M -Xms1024M -jar server.jar nogui.

   4. Discord Webhook — ссылка на вебхук вашего канала для получения отчетов.

   5. CurseForge API Key — ключ для поиска плагинов (можно получить на console.curseforge.com).
